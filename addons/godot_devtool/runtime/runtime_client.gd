...
const RuntimeStateStore := preload("res://addons/godot_devtool/runtime/runtime_state_store.gd")
...
var _state_store := RuntimeStateStore.new()
...
func _load_config() -> void:
	...
	if typeof(parsed) == TYPE_DICTIONARY:
		...
		var env_run_id := OS.get_environment("GODOT_DEVTOOL_RUN_ID")
		_run_id = env_run_id if env_run_id != "" else str(parsed.get("runId", _run_id))
