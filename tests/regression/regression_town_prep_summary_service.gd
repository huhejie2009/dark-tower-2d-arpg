...
func _run() -> void:
	...
	_expect(str(summary.get("character_text", "")).contains("等级7"), "summary should include player level")
	...
	_expect(str(summary.get("growth_text", "")).contains("天赋点 3"), "summary should include skill points")
