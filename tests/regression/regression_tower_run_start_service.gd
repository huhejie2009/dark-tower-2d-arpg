...
func _run() -> void:
	...
	_expect(str(options.get("fresh_description", "")).contains("新的爬塔"), "fresh option should explain it starts a new climb")
	_expect(str(options.get("best_description", "")).contains("已保存进度"), "best option should explain saved progress")
	_expect(str(options.get("highest_floor_explanation", "")).contains("最高层"), "start options should explain highest floor")
