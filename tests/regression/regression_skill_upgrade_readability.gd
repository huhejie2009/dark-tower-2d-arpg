...
func _run() -> void:
	...
	if window.has_method("get_basic_attack_upgrade_preview_for_test"):
		...
		_expect(str(preview.get("summary_text", "")).contains("伤害 +3"), "preview summary should describe gain")
		_expect(str(preview.get("status_text", "")).contains("可以升级"), "preview status should be readable")
