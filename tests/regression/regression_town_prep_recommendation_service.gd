...
func _run() -> void:
	...
	_expect(str(result.get("recommendation_text", "")).contains("技能点 2"), "recommendation text should mention unspent skill points")
	_expect(str(result.get("recommendation_text", "")).contains("更强装备"), "recommendation text should mention equipment upgrades")
	_expect(str(result.get("recommendation_text", "")).contains("背包"), "recommendation text should mention bag pressure")
	...
	_expect(_has_recommendation_action(items, "spend_skill_points", "open_skills", "打开技能"), "skill recommendation should expose skill action")
	...
	_expect(str(result.get("primary_button_text", "")) == "打开技能", "primary action should expose button text")
	...
	_expect(str(clean_result.get("recommendation_text", "")).contains("准备就绪"), "clean recommendation should say ready")
