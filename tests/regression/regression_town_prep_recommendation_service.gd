...
func _run() -> void:
	...
	_expect(str(result.get("recommendation_text", "")).contains("2 点天赋点"), "recommendation text should mention unspent skill points")
	_expect(str(result.get("recommendation_text", "")).contains("更强装备"), "recommendation text should mention equipment upgrades")
	_expect(str(result.get("recommendation_text", "")).contains("背包压力"), "recommendation text should mention bag pressure")
	...
	_expect(_has_recommendation_action(items, "spend_skill_points", "open_skills", "打开天赋"), "skill recommendation should expose skill action")
	_expect(_has_recommendation_action(items, "equip_upgrade", "open_equipment", "打开装备"), "equipment recommendation should expose equipment action")
	_expect(_has_recommendation_action(items, "manage_bag", "open_inventory", "打开背包"), "bag recommendation should expose inventory action")
	_expect(str(result.get("primary_action_id", "")) == "open_skills", "highest priority recommendation should become primary action")
	_expect(str(result.get("primary_button_text", "")) == "打开天赋", "primary action should expose button text")
	...
	_expect(str(clean_result.get("recommendation_text", "")).contains("准备完毕"), "clean recommendation should say ready")
