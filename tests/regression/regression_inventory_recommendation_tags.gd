...
func _run() -> void:
	...
	if window.has_method("get_item_recommendation_for_test"):
		...
		_expect(str(recommendation.get("source_label", "")) == "首领奖励", "recommendation should expose readable boss source")
	...
	_expect(str(visual_meta.get("source_label", "")) == "首领奖励", "visual metadata should expose source label")
	...
	_expect(detail.contains("来源：首领奖励"), "detail should include readable source")
	...
	_expect(detail.contains("评分变化：+"), "detail should include signed score delta")
